...
{
	clk.setColorDepth(8);									   // 设置颜色像素位数（1 4 8 16）
	clk.createSprite(TFT_WIDTH - 4, 60);					   // 创建窗口
	clk.fillSprite(BAKCGROUND_COLOR);						   // 填充
	clk.loadFont(My_Font);									   // 加载显示的字体（自制的字库）
	clk.setTextDatum(CC_DATUM);								   // 设置文本数据
	clk.setTextColor(PEN_COLOR, BAKCGROUND_COLOR);			   // 设置字体显示区域的颜色，含背景颜色和字体颜色
	clk.drawString("WiFi 连接失败!", (TFT_WIDTH - 4) / 2, 10); // 指定文字显示在画布的指定位置
	clk.drawString("请自行连接:", (TFT_WIDTH - 4) / 2, 30);
	clk.setTextColor(TFT_WHITE, BAKCGROUND_COLOR);
	clk.drawString("SSID:", (TFT_WIDTH - 4) / 2 - 40, 50);
	clk.drawString(My_AP, (TFT_WIDTH - 4) / 2, 50);
	clk.pushSprite(2, 49); // 窗口位置
	clk.deleteSprite();	   // 删除画布
	clk.unloadFont();	   // 卸载字体，释放资源（RAM）
}
...
void weater_Show(String *cityDZ, String *dataSK, String *dataFC)
{
	...
	// 天气图标
	wrat.printfweather(50, 15, atoi((sk["weathercode"].as<String>()).substring(1, 3).c_str()));
...
void digitalClockDisplay(int reflash_en)
{
	...
	{
		drawLineFont(10, timeY, now_hour / 10, 2, SD_FONT_WHITE);
		drawLineFont(40, timeY, now_hour % 10, 2, SD_FONT_WHITE);
		Hour_sign = now_hour;
	}
	...
	{
		drawLineFont(70, timeY, now_minute / 10, 2, SD_FONT_YELLOW);
		drawLineFont(100, timeY, now_minute % 10, 2, SD_FONT_YELLOW);
	...
	{
		drawLineFont(110, timeY + 30, now_second / 10, 1, SD_FONT_WHITE);
		drawLineFont(120, timeY + 30, now_second % 10, 1, SD_FONT_WHITE);
...
void DHT11_Show()
{
	...
	clk.fillSprite(BAKCGROUND_COLOR);
