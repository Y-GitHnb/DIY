 ...
 */
void ATK_IMD750_Init(void)
{
    uint8_t flag;       //返回标志
    
g4_init:
    atk_idm750c_dtu_enter_configmode();     //DTU进入配置状态
    flag = atk_idm750c_at_test();       /* ATK-IDM750C AT指令测试 */
    if (flag != ATK_IDM750C_EOK)      // 有错
    {
        OLED_ShowString_fun("4G Init ERROR");
        printf("ATK-IMD750 Init ERROR\r\n");
        goto g4_init;
    }
    OLED_ShowString_fun("4G Init OK");
    printf("ATK-IMD750 Init OK\r\n");
    
    flag = atk_idm750c_query_workmode(ATK_IDM750C_WORKMODE_NET);   //工作模式设置：网络透传模式
    flag += atk_idm750c_link1en(ATK_IDM750C_LINK1EN_ON);    //第一路连接使能状态：ON
    flag += atk_idm750c_link1(ATK_IDM750C_LINK1MODE_TCP,DEMO_DTU_TCP_SERVER_DOMAIN_NAME,DEMO_DTU_TCP_SERVER_PORT_NUMBER);   //第一路连接参数
    flag += atk_idm750c_link1md(ATK_IDM750C_LINK1MD_LONG);  //第一路连接模式：长连接
    flag +=atk_idm750c_link1_timeout();                     //第一路超时重连时间
    flag += atk_idm750c_link2en(ATK_IDM750C_LINK2EN_OFF);   //第二路连接使能状态：OFF
    flag += atk_idm750c_link3en(ATK_IDM750C_LINK3EN_OFF);   //第三路连接使能状态：OFF
    flag += atk_idm750c_link4en(ATK_IDM750C_LINK4EN_OFF);   //第四路连接使能状态：OFF
    flag += atk_idm750c_set_yuanziyun(ATK_IDM750C_SET_YUANZIYUN_ON);            //启用原子云
    flag += atk_idm750c_device_num(DEMO_DTU_TCP_YUANZI_DEVICE_NUMEBER);         //原子云设备编号
    flag += atk_idm750c_device_password(DEMO_DTU_TCP_YUANZI_DEVICE_PASSWORD);   //原子云设备密码
    flag += atk_idm750c_heartbeat_package(ATK_IDM750C_HRTEN_ON);    //开启心跳包
    flag += atk_idm750c_heartbeat_package_data();                   //心跳包数据
    flag += atk_idm750c_heartbeat_package_interval();               //心跳包时间间隔
    flag += atk_idm750c_registration_package(ATK_IDM750C_SET_REGEN_OFF);    //注册包默认关闭
    flag += atk_idm750c_registration_package_data();                        //注册包数据
    flag += atk_idm750c_registration_package_send_method();                 //注册包发送方式
    flag += atk_idm750c_registration_package_data_type();                   //注册包数据类型
    flag += atk_idm750c_enter_transfermode();    //进入透传状态
    if (flag != 0)
    {
        OLED_ShowString_fun("4G NET. ERROR");
        printf("ATK-IMD750 NET. ERROR\r\n");
        goto g4_init;
    }
    OLED_ShowString_fun("4G NET. OK");
    printf("ATK-IMD750 NET. OK\r\n");
